...
async function worker(options) {
  ...
  const normalizeProcessedResult = (item, filename) => {
    ...
    if (typeof filename !== "undefined" && typeof options.generateFilename !== "undefined" && !item.info.original) {
  ...
  for (let i = 0; i <= transformers.length - 1; i++) {
    if (transformers[i].filter && // @ts-ignore
    !transformers[i].filter(
    /** @type {Buffer} */
    options.input, options.filename)) {
      continue;
    }
    /** @type {WorkerResult} */
    ...
    try {
      ...
      processedResult = await transformers[i].implementation(result, transformers[i].options);
    } catch (error) {
    ...
    result = normalizeProcessedResult(processedResult, transformers[i].filename);
  }
